...
object QuarterlyTrendAnalyzer {
    ...
    fun summarize(history: QuarterlyHistory): String {
        ...
        val marginSeries = usable.filter { it.operatingMargin != null }
        val marginTrend = direction(marginSeries.takeLast(4).mapNotNull { it.operatingMargin })
        val fourQuarterAgo = usable
            .dropLast(1)
            .lastOrNull { it.fiscalYear == latest.fiscalYear - 1 && it.quarter == latest.quarter }
        val comparableYoy = if (fourQuarterAgo != null && latest.revenue != null && fourQuarterAgo.revenue != null) {
            safePct(latest.revenue, fourQuarterAgo.revenue)
        } else null
        val previous = usable.dropLast(1).lastOrNull()
        val comparableQoq = if (previous != null && latest.revenue != null && previous.revenue != null &&
            quarterDistance(previous, latest) == 1
        ) {
            safePct(latest.revenue, previous.revenue)
        } else null
        ...
        return buildString {
            append("동일 회계범위(${history.comparisonScope.ifBlank { "확인가능 범위" }}) 실제 분기 ${usable.size}개를 기준으로 보면 매출 흐름은 $revenueTrend")
            if (marginSeries.size >= 3) append(", 영업이익률 흐름은 $marginTrend")
            append("입니다. ")
            append("최신 ${latest.period} 매출은 ${formatAmount(latest.revenue)}")
            comparableYoy?.let { append(", 전년동기 대비 ${formatPct(it)}") }
            comparableQoq?.let { append(", 전분기 대비 ${formatPct(it)}") }
            append("입니다. ")
            ...
            append(" 이 시계열은 OpenDART 실제 손익계산서에서 직접 3개월 값을 우선 사용하고, 직접값이 없을 때만 같은 CFS/OFS 범위의 누적 차감으로 복원합니다. 회계범위가 바뀐 분기는 추세 계산에서 제외합니다.")
        }
    }
    ...
    fun compactSignal(history: QuarterlyHistory): String {
        ...
        val revenue = points.mapNotNull { it.revenue }
        val margin = points.mapNotNull { it.operatingMargin }
    ...
    private fun quarterDistance(a: QuarterlyPoint, b: QuarterlyPoint): Int =
        (b.fiscalYear * 4 + b.quarter) - (a.fiscalYear * 4 + a.quarter)
