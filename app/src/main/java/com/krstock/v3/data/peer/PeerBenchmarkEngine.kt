 ...
 * Exact KRX sector is preferred. A transparent standard sector family is used
 * only when the exact sector has too few metric-specific observations.
 */
object PeerBenchmarkEngine {
    const val MIN_EXACT_SAMPLE: Int = 8
    const val MIN_FAMILY_SAMPLE: Int = 20

    private data class MetricSpec(val id: String, val read: (StockSummary) -> MetricValue)
    ...
    private val specs = listOf(
        MetricSpec("M01") { it.m01RevGrowth },
        MetricSpec("M02") { it.m02OpMargin },
        MetricSpec("M03") { it.m03Per },
        MetricSpec("M04") { it.m04Price6m }
    )
        ...
        stocks.forEach { stock ->
            ...
            specs.forEach { spec ->
                ...
                if (raw != null && raw.isFinite()) {
                    exactSector?.let {
                        exactValues.getOrPut(BucketKey(it, spec.id)) { mutableListOf() }.add(raw)
                    }
                    family?.let {
                        familyValues.getOrPut(BucketKey(it.key, spec.id)) { mutableListOf() }.add(raw)
                    }
                }
            }
        }

        val exactDistributions = exactValues.mapValues { Distribution(it.value.sorted()) }
        val familyDistributions = familyValues.mapValues { Distribution(it.value.sorted()) }
            ...
            fun benchmark(metric: MetricValue, higherIsBetter: Boolean): PeerComparison {
                ...
                if (selected == null) {
                    ...
                    return PeerComparison(
                        ...
                        reason = if (exactSector == null && family == null) {
                            "비교 가능한 업종 분류가 없어 피어 중앙값을 만들지 않음"
                        } else {
                            "피어 표본 부족 · 세부업종 ${exact?.size ?: 0}개 / 표준그룹 ${broad?.size ?: 0}개"
                        }
                    )
                }
    ...
    private fun normalizedExactSector(raw: String): String? {
        ...
        return value.takeUnless { it.isBlank() || it == "기타" || it == "미분류" || it == "없음" }
    }
    ...
    private fun classifyFamily(raw: String): SectorFamily? {
        ...
        return when {
            has("은행", "금융", "보험", "증권", "신탁", "여신", "신용카드") -> SectorFamily("FINANCE", "금융·보험")
            ...
            has("기계", "공작기계", "특수목적", "일반목적", "산업용로봇", "펌프", "밸브", "베어링") -> SectorFamily("MACHINERY", "기계·산업장비")
            has("철강", "1차철강", "비철금속", "알루미늄", "금속가공", "구조용금속", "압연") -> SectorFamily("METAL", "철강·금속")
            has("화학", "합성수지", "합성고무", "플라스틱", "고무제품", "도료", "접착", "비료") -> SectorFamily("CHEMICAL", "화학·소재")
            has("시멘트", "콘크리트", "유리", "도자기", "비금속광물", "골재", "건축자재") -> SectorFamily("BUILDING_MATERIAL", "건자재")
            has("건설", "건축", "토목", "건축기술", "엔지니어링") -> SectorFamily("CONSTRUCTION", "건설·엔지니어링")
            has("석유", "정유", "원유", "석탄", "연료", "가스제조") -> SectorFamily("ENERGY", "에너지·정유·가스")
            ...
            has("도매", "소매", "유통", "전자상거래", "백화점", "편의점") -> SectorFamily("RETAIL_DISTRIBUTION", "유통·도소매")
            has("섬유", "의복", "봉제", "가죽", "신발") -> SectorFamily("TEXTILE_APPAREL", "섬유·의류")
            has("화장품", "가구", "생활용품", "레저", "여행", "숙박", "음식점") -> SectorFamily("CONSUMER", "소비재·레저")
            has("종이", "펄프", "포장", "골판지") -> SectorFamily("PAPER_PACKAGING", "종이·포장")
            has("연구개발", "전문서비스", "컨설팅", "교육", "사업지원", "인력공급") -> SectorFamily("PROFESSIONAL_SERVICE", "전문·사업서비스")
            has("제조업", "제조") -> SectorFamily("OTHER_MANUFACTURING", "기타 제조")
            has("서비스업", "서비스") -> SectorFamily("OTHER_SERVICE", "기타 서비스")
            else -> null
        }
    }
}
