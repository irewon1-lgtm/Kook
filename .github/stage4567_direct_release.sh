...
STABLE_BASE_SHA="18e9d176c3fd28e5150e138dee748f17e2eb16ef"
...
assert isinstance(cs, list) and len(cs) == 10, type(cs)
codes = [str(x.get('code')) for x in cs]
expected = ['025560','187870','005950','000660','013700','059100','031330','037460','115480','080220']
assert codes == expected, codes
eligible = fc.get('eligible_count', fc.get('eligible'))
if eligible is not None:
    assert int(eligible) == 1366, eligible
schema = fc.get('schema') or fc.get('schema_version') or fc.get('schema_name')
if schema is not None:
    assert 'V2' in str(schema).upper(), schema
print('DIRECT_PREPARE_DATA_PASS', {'stability': [p,f,h,n,t], 'eligible': eligible, 'schema': schema, 'candidates': codes})
PY
...
PY
  ...
  echo "KR4_VERSION_CODE=$NEXT" >> "$GITHUB_ENV"
...
build() {
  ...
  test -s "$APK"
  mkdir -p dist evidence/ci/stage4567
  cp "$APK" dist/KR4.apk
  ...
  ACTUAL=$(awk -F': ' '/Signer #1 certificate SHA-256 digest/{print tolower($2); exit}' dist/APK_SIGNING_CERT.txt | tr -d ':')
  ...
  sha256sum dist/KR4.apk | tee evidence/ci/stage4567/APK_SHA256.txt
  stat --printf='APK_BYTES=%s\n' dist/KR4.apk | tee evidence/ci/stage4567/APK_SIZE.txt
...
manifest() {
  test -n "${PROMOTED_SHA:-}"; test -s dist/KR4.apk
  SHA=$(sha256sum dist/KR4.apk | awk '{print $1}'); export SHA
  python3 - <<'PY'
import json, os
...
publish() {
  for f in dist/KR4.apk dist/update.json dist/APK_SIGNING_CERT.txt; do test -s "$f"; done
  test -n "${PROMOTED_SHA:-}"; test -n "${KR4_RELEASE_TAG:-}"
  if gh release view "$KR4_RELEASE_TAG" >/dev/null 2>&1; then
    echo "release tag collision: $KR4_RELEASE_TAG" >&2
    exit 1
  fi
  TITLE="KR4 ${KR4_VERSION_NAME}"
  NOTES="Stage4 재무안정성 + Stage5 실적 PER 상대밴드 + Stage6/7 최종후보 V2. pre-CLEAN 및 Android 14 E2E 통과. commit ${PROMOTED_SHA}."
  gh release create "$KR4_RELEASE_TAG" dist/KR4.apk dist/update.json dist/APK_SIGNING_CERT.txt --target "$PROMOTED_SHA" --title "$TITLE" --notes "$NOTES" --latest
  ...
  gh release download "$KR4_RELEASE_TAG" -p KR4.apk -p update.json -p APK_SIGNING_CERT.txt -D "$RUNNER_TEMP/kr4-release-verify"
...
assert d['commit']==os.environ['PROMOTED_SHA']
assert d['versionCode']==int(os.environ['KR4_VERSION_CODE'])
assert d['signingCertificateSha256']==os.environ['KR4_SIGNING_CERT_SHA256']
...
case "${1:-}" in
  ...
  manifest) manifest ;;
  publish) publish ;;
  *) echo "usage: $0 {prepare|sign|build|promote|manifest|publish}" >&2; exit 2 ;;
esac
