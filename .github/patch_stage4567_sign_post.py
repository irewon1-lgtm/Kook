...
old_cert = '''  ACTUAL=$(awk -F': ' '/Signer #1 certificate SHA-256 digest/{print tolower($2); exit}' dist/APK_SIGNING_CERT.txt | tr -d ':')
  test "$ACTUAL" = "$EXPECTED_CERT"'''
new_cert = '''  ACTUAL=$(python3 - <<'PYAPK'
import re
text=open('dist/APK_SIGNING_CERT.txt',encoding='utf-8',errors='replace').read()
m=re.search(r'(?im)certificate SHA-256 digest:\\s*([0-9a-f:]{64,95})\\s*$', text)
assert m, 'apk_certificate_sha256_missing'
print(m.group(1).replace(':','').lower())
PYAPK
)
  test "$ACTUAL" = "$EXPECTED_CERT"
  echo "APK_CERT_PARSE_PASS $ACTUAL"'''
cert_count = text.count(old_cert)
assert cert_count == 1, f'expected exactly one legacy APK cert parser, found {cert_count}'
text = text.replace(old_cert, new_cert)
assert 'Signer #1 certificate SHA-256 digest' not in text
assert 'APK_CERT_PARSE_PASS' in text

path.write_text(text, encoding='utf-8')
print('SIGNING_BROWSER_RELAY_AND_APK_CERT_PATCH_PASS')
